import AfterLoginHeader from "../../../components/Header/AfterLoginHeader"
import { 
	Container, 
	Text, 
	Flex, 
	Box, 
	HStack,
	VStack,
	Stack, 
	Avatar,
	Heading,
	Button,
	Link,
	Badge
} from '@chakra-ui/react'

import {SearchIcon, CalendarIcon} from "@chakra-ui/icons"



const ArtistBooking = () => {

	return(
		<>
			<AfterLoginHeader/>

			<Box bg='neutral.100' p={[4, '100px']}>
		  		<Container maxW="750px" px="15px" pt="20px" pb="50px" bg="white" borderRadius="5px">
		  			<Flex justifyContent="space-between" alignItems="center">
		  				<Text>BOOKINGS</Text>	

		  				<Flex w="40px" h="40px" bg="#00B0F3" borderRadius="50%" alignItems="center" justifyContent="center">
		  					<SearchIcon color="white"/>
		  				</Flex>
		  			</Flex>

		  			<Stack mt="20px" spacing={4}>
		  				<Flex bg="gray.200" px="10px" py="30px" justifyContent="space-between" gap="20px" flexDirection={{base:"column", md:"row"}}>
		  					<HStack spacing={4} flexDirection={{base:"column", md:"row"}}>
		  						<Avatar
		  							w="100px"
		  							h="100px"
		  							src="data:image/jpeg;base64,"
		  						/>
		  						
	  							<VStack alignItems={{base:"center", md:"flex-start"}} spacing={0.5}>
	  								<Heading fontSize="1rem" textAlign={{base:"center"}} color="primary">2 MINUTES AFROBEATS HAPPY SONG</Heading>
	  								<Text fontSize="0.85rem" as="b">2 Minutes Song</Text>
	  								<Text fontSize="0.85rem">Client: <Text as="b">Adedoyin Dolapo</Text></Text>
	  								<Text as="span" fontSize="0.85rem"><CalendarIcon/> August 12, 2023</Text>
	  							</VStack>
		  							
		  						
		  					</HStack>

		  					<VStack spacing={1}>
		  						<HStack>
		  							<Button alignSelf="center" colorScheme="green" variant="solid" fontWeight="500" as="a">Accept</Button>
		  							<Button alignSelf="center" colorScheme="red" variant="solid" fontWeight="500" as="a">Reject</Button>
		  						</HStack>
		  						
		  						<Button alignSelf="center" variant="ghost" color="primary" fontWeight="500" as="a">Message Client</Button>
		  					</VStack>
		  					
		  				</Flex>

		  				<Flex bg="gray.200" px="10px" py="30px" justifyContent="space-between" gap="20px" flexDirection={{base:"column", md:"row"}}>
		  					<HStack spacing={4} flexDirection={{base:"column", md:"row"}}>
		  						<Avatar
		  							w="100px"
		  							h="100px"
		  							src="data:image/jpeg;base64,"
		  						/>
		  						
	  							<VStack alignItems={{base:"center", md:"flex-start"}} spacing={0.5}>
	  								<Heading fontSize="1rem" textAlign={{base:"center"}} color="primary">2 MINUTES AFROBEATS HAPPY SONG</Heading>
	  								<Text fontSize="0.85rem" as="b">2 Minutes Song</Text>
	  								<Text fontSize="0.85rem">Client: <Text as="b">Adedoyin Dolapo</Text></Text>
	  								<Text as="span" fontSize="0.85rem"><CalendarIcon/> August 12, 2023</Text>
	  							</VStack>
		  							
		  						
		  					</HStack>

		  					<VStack spacing={1}>
		  						<Button alignSelf="center" fontWeight="500" as="a">View Details</Button>
		  						<Button alignSelf="center" variant="ghost" color="primary" fontWeight="500" as="a">Active</Button>
		  					</VStack>
		  					
		  				</Flex>

		  			</Stack>
		  		</Container>
		  	</Box>
		</>
	)
}

export default ArtistBooking